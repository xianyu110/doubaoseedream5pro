...
- URL-prefix 资源：`http://doubaoseedream5pro.cc/`
- 验证方式：HTML 文件
- 验证文件：`google6f7d8765f4c7bf70.html`
- Sitemap：`http://doubaoseedream5pro.cc/sitemap.xml`

## Google Search Console RPA

仓库内置了一个本地 RPA 工具，用可见 Chrome 自动完成 GSC URL-prefix 资源添加、HTML 文件验证和 Sitemap 提交。它不会保存 Google 密码，也不会读取系统浏览器的 Cookie；登录、二次验证、验证码和账号授权提示需要人工在浏览器里完成。
...
```bash
npm run gsc:add -- \
  --site http://doubaoseedream5pro.cc/ \
  --sitemap sitemap.xml \
  --verification-file google6f7d8765f4c7bf70.html
```

如果站点已经在 GSC 验证过，只补交 Sitemap：
...
默认使用一次性浏览器会话，运行结束后登录状态会丢弃。如果希望保留一个专用的本地登录状态，可以加 `--profile .gsc-rpa-profile`；该目录已加入 `.gitignore`，不要提交到 GitHub。
...
限制说明：Google Search Console 页面结构可能变化，脚本找不到输入框或按钮时会暂停并保存截图到 `.gsc-rpa-output/`，这时按提示手动完成当前页面动作后回到终端按 Enter 即可继续。
